...
# Step 0.1: Project Intake Card (ChatGPT)
# v7 SOP 6.1 [S0] — PI / Research Architect role, 6 specific actions
STEP_0_1_PROMPT = """You are a PI / Research Architect.

## Task
Create the Project Intake Card for the research project described below.
...
Define ONE clear sentence: what strictly are we proving?
...
- **Hard Constraints (red lines)**: non-negotiable requirements that cannot be crossed.
- **Soft Constraints (preferences)**: desirable but flexible requirements.

### 3. Definition of Done (DoD)
Minimum required deliverables (≥3 items). Each item should be specific and measurable — e.g., figures, tables, claims, experiments, benchmarks.

### 4. Top 10 Risks + Mitigations
List top 10 risks ranked by severity × probability. Each mitigation must be an actionable step, not a platitude.

### 5. Keyword Bundles for Deep Research
Provide 5 keyword bundles, each with English and Chinese versions:
- Bundle 1 (EN): ... | (CN): ...
- Bundle 2 (EN): ... | (CN): ...
- ...

### 6. Scope Boundary
Define what is explicitly **OUT of scope** — things we will NOT do in this project.

## Output Format
File: `00_Project_Intake_Card.md`
...
Then provide the 6 sections above. Be specific and actionable.
"""


# ============================================================
# LEGACY: Step 0.1 Prompt (SOP v4.0) — 保留用于对比
# ============================================================
# STEP_0_1_PROMPT_V4_LEGACY = """You are an experienced research PI and paper architect. Your task is to create a comprehensive Project Intake Card for a new research project.
#
# ## Input Information:
# - **Topic**: {topic}
# - **Target Venue**: {target_venue}
...
# v7 SOP 6.2 [S0b] — Senior Editor role, 4 specific actions
STEP_0_2_PROMPT = """You are a Senior Editor of {target_venue}.

## Role
Calibrate Taste & Narrative Expectations for the research topic below.

## Input
- Topic: {topic}
- Target Venue: {target_venue}
- (The Project Intake Card is attached below for full context.)

## Actions (complete all four)

### 1. REWARD Patterns
Summarize what {target_venue} tends to **REWARD** — e.g., methods/mechanism insight, clinical impact, causal rigor, reproducibility, novelty of formulation. Provide evidence (recent accepted papers, editorial statements, or call-for-papers language) for each claim.

### 2. REJECT Patterns
Summarize what {target_venue} tends to **REJECT** — e.g., black-box prediction without explanation, weak baselines, unclear endpoints, incremental improvements, missing ablations. Cite concrete rejection patterns or reviewer feedback trends where possible.

### 3. Reviewer #2 Attack Angles
List the top 15 **"Reviewer #2 Attack Angles"** most relevant to this topic at this venue. Each angle should be a specific, actionable critique a tough reviewer might raise. Number them 1-15.

### 4. Paper Identity Options
Propose exactly 3 **"Paper Identity"** framings:
- **Option A — Mechanism-First**: emphasize why/how it works
- **Option B — Methods-First**: emphasize technical novelty of the approach
- **Option C — Impact-First**: emphasize practical significance and downstream value

For each option, give a one-sentence pitch and list pros/cons for this venue. Then **recommend one** with a brief justification.

## Output Format
- File: `00_Venue_Taste_Notes.md`
- Style: concise, bullet-first, with evidence for each claim about the venue
- Begin with YAML front-matter:

```yaml
---
doc_type: VenueTasteNotes
version: 0.1
status: draft
created_by: Gemini
target_venue: "{target_venue}"
...
Then provide the 4 sections above. Keep it focused and actionable.
"""


# ============================================================
# LEGACY: Step 0.2 Prompt (SOP v4.0) — 保留用于对比
# ============================================================
# STEP_0_2_PROMPT_V4_LEGACY = """You are an expert research intelligence officer specializing in academic venue analysis. Your task is to analyze the target venue and provide detailed taste notes.
#
