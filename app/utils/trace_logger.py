...
class TraceLogger:
    ...
    def save_raw_response(self, project_id: str, step_id: str,
                         response: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        保存原始 AI 响应（在任何处理之前）
        ...
        try:
            ...
            logger.info(f"✓ Saved raw response to {trace_file} ({len(response)} chars)")
    ...
    def load_raw_response(self, project_id: str, step_id: str,
        ...
        if timestamp:
            ...
            # 找最新的文件
            files = list(trace_dir.glob(f"{step_id}_raw_*.md"))
            if not files:
        ...
        # 如果没有找到标记，返回整个内容
        logger.warning("⚠️ Could not find '## Raw Response' marker, returning full content")
        return content
