...
class Settings(BaseSettings):
    ...
    vector_db_path: str = Field(default="./data/chroma", description="ChromaDB 数据目录 → 挂载 /app/data/chroma")
    projects_path: str = Field(default="./data/files", description="业务文件 JSON/MD 目录 → 挂载 /app/data/files")
    ...
    # 认证（生产必须设置环境变量 SECRET_KEY，禁止使用默认值）
    ...
    cors_origins: str = Field(default="http://localhost:5173", description="允许的源，逗号分隔，如 https://app.example.com")
    ...
    gemini_gem_config_path: Optional[str] = Field(default=None, description="Gemini Gem 配置路径，可选，可挂载进容器")
